// ============================================================
// ADOS - Seguir Contando Contigo
// Formulario con desplegables en cascada
// ============================================================
...
// URL del Apps Script (actualizar tras desplegar)
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwsd7LtlhJtybVTcX7x3EtqTXIEbz7pshds6NpwDWBvvGjbpeKxjfCOmvC1ErNIrb9m9g/exec';
...
function initForm() {
  if (!TOKEN) {
    ...
    const frm = document.getElementById('frmDonante');
  ...
  selProvincia.innerHTML = '<option value="">-- Selecciona provincia --</option>';
  ...
  // Resetear municipio y lugar
  const selMunicipio = document.getElementById('municipio');
  const selLugar     = document.getElementById('lugar');
  if (selMunicipio) { selMunicipio.innerHTML = '<option value="">-- Primeiro selecciona provincia --</option>'; selMunicipio.disabled = true; }
  if (selLugar)     { selLugar.innerHTML     = '<option value="">-- Primeiro selecciona municipio --</option>'; selLugar.disabled = true; }
}
...
document.addEventListener('change', function(e) {
  if (e.target.id !== 'provincia') return;
...
document.addEventListener('change', function(e) {
  if (e.target.id !== 'municipio') return;
  ...
  const key   = `${provId}_${munId}`;
  const sites = COLLECTSITES[key] || [];
...
document.addEventListener('submit', function(e) {
  if (e.target.id !== 'frmDonante') return;
  e.preventDefault();
